...
public class ConverterPontosController implements Initializable {
    ...
    public ConverterPontosController() {
        ...
        this.loggedInUser.setIdUsuario(1); // Assumindo ID do usuário logado
    ...
    private void loadEcoPoints() {
        ...
        try {
            ...
            if (ecoPontosOpt.isPresent()) {
                ...
                ecopontosToConvertLabel.setText(ecoPontosOpt.get().getTotalPontos() + " Ecopontos");

            } else {

                ecopontosToConvertLabel.setText("0 Ecopontos");
            ...
            showAlert(Alert.AlertType.ERROR, "Erro Geral", "Ocorreu um erro ao carregar os ecopontos: " + e.getMessage());
    ...
    @FXML
    public void onConvertNowClick() {

        try {

            boolean podeTrocar = certificadosDAO.podeTrocarPorCertificado(loggedInUser.getIdUsuario(), POINTS_REQUIRED_FOR_CERTIFICATE);


            if (podeTrocar) {

                Optional<EcoPontos> ecoPontosOpt = Optional.ofNullable(ecoPontosDAO.findByUsuario(loggedInUser.getIdUsuario())); if (ecoPontosOpt.isPresent()) {

                    EcoPontos userEcoPontos = ecoPontosOpt.get();
                    ...
                    newCertificado.setIdPonto(userEcoPontos.getIdPonto());

                    newCertificado.setDescricao("Embaixador da Sustentabilidade");
                    ...
                    App.getInstance().loadScene("converter_pontos_sucesso.fxml");
