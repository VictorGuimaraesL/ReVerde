...
public class ConverterPontosSucessoController implements Initializable {
    ...
    public void onDownloadCertificateClick(ActionEvent event) {
        ...
        fileChooser.setInitialFileName("CertificadoEmbaixadorSustentabilidade_" + LocalDate.now() + ".png"); // Alterado para .png
        ...
        if (file != null) {
            try (FileOutputStream fos = new FileOutputStream(file)) {
                if (certificateImageData != null) {
                    ...
                    // Se não houver dados de imagem, pode gerar uma imagem placeholder ou lançar um erro
                    System.err.println("Erro: Dados da imagem do certificado não disponíveis.");
                    // Poderia carregar uma imagem padrão ou exibir um alerta
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
